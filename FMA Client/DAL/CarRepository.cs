...
namespace DAL
{
    public class CarRepository : ICarRepository
    {
            ...
            using (SqlCommand command = connection.CreateCommand())
            {
                try
                {
                    ...
                    while (datareader.Read())
                    {
                        ...
                        if (DBNull.Value != datareader["DriverId"])
                        {
                            ...
                            if (DBNull.Value != datareader["FuelcardId"])
                            {
                                ...
                                */
                                ...
                                fuelcard = new(fuelcardId, cardnumber, expiryDate, /*pincode,*/ fueltypesListFuelcard,
                                    isActive);
                            }
                            ...
                            driver = new(driverId, firstName, lastName, address, dateOfBirth,
                                nationalIdentificationNumber, licensesList, null, fuelcard);
                        }
                        ...
                        Car car = new(carId, make, model, vin, licenseplate, vehicleType, fueltypesList, driver, colour,
                            doors);
        ...
        //Done
        public IReadOnlyList<Car> GetCars(int? carId, string vin, string licenseplate, string make, string model,
            string vehicleType
            ...
            using (SqlCommand command = connection.CreateCommand())
            {
                try
                {
                    ...
                    while (datareader.Read())
                    {
                        ...
                        Car car = new(carIdDb, makeDb, modelDb, vinDb, licenseplateDb, vehicleTypeDb, fueltypesList,
                            driver, colourDb, doorsDb);
            ...
            , List<Fuel> fueltypes, string doors, string colour)
        {
            ...
            SqlConnection connection = getConnection();
            string query =
                "INSERT INTO dbo.Car ([Vin],[Licenseplate],[Make],[Model],[VehicleType],[Fueltypes],[Doors],[Colour])" +
        ...
        public IReadOnlyList<Car> Search(string x)
        {
            ...
            string query =
                "SELECT *, C.Fueltypes AS carFueltypes, F.Fueltypes AS fuelcardFueltypes From dbo.Car AS C " +
                " LEFT JOIN dbo.Driver AS D ON D.CarId = C.CarId" +
                " LEFT JOIN dbo.Fuelcard AS F ON F.FuelcardId = D.FuelcardId" +
                " LEFT JOIN dbo.Address AS A ON A.AddressId = D.AddressId" +
                " WHERE @searchterm in (Vin, Licenseplate, make, model, vehicleType, C.Fueltypes, Doors, Colour)";
